...
create or replace FUNCTION DIRECTION_LIST_MANAGEMENT(
    p_user_id NUMBER  -- El ID del usuario, que se recibirï¿½ siempre
) RETURN SYS_REFCURSOR
IS
    v_cursor SYS_REFCURSOR;
BEGIN
    -- Abrir un cursor para seleccionar todos los pagos donde ACTIVE sea igual a 1
    -- y el USER_ID sea el dueï¿½o del pago
    OPEN v_cursor FOR
    SELECT *
    FROM DIRECTIONS
    WHERE ACTIVE = 1
    AND USER_ID = p_user_id;  -- Compara si el ID que llega es del dueï¿½o del pago
...
create or replace FUNCTION GET_ALL_PRODUCTS(
    p_user_id NUMBER := NULL  -- Parï¿½metro opcional, por defecto NULL
) RETURN SYS_REFCURSOR
IS
    v_cursor SYS_REFCURSOR;
BEGIN
...
BEGIN
    ...
    SELECT *
    FROM banking_entities
    WHERE ACTIVE = 1;  -- Compara si el ID que llega es del dueï¿½o del pago
    RETURN v_cursor; -- Retornar el cursor con los registros
EXCEPTION
    WHEN OTHERS THEN
        -- Manejo de otras excepciones
...
IS
    v_cursor SYS_REFCURSOR;
BEGIN
    -- Abrir un cursor con el registro del usuario que coincide por email y estï¿½ activo
    OPEN v_cursor FOR
    SELECT *
...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        -- Manejo de excepciï¿½n si no se encuentra el usuario
        RETURN NULL;
    WHEN OTHERS THEN
        -- Manejo de otras excepciones
        RAISE;
...
BEGIN
    -- Asegï¿½rate de usar un filtro que garantice una ï¿½nica fila
...
EXCEPTION
    ...
    WHEN TOO_MANY_ROWS THEN
        -- Maneja el caso en que se devuelvan mï¿½ltiples filas (opcional)
        RETURN NULL;
    WHEN OTHERS THEN
        RAISE;
END GET_PASSWORD;
...
BEGIN
    ...
    FROM PRODUCTS p
    INNER JOIN USERS u ON u.user_id = p.user_id;  -- Compara si el ID que llega es del dueï¿½o del pago
    RETURN v_cursor; -- Retornar el cursor con los registros
EXCEPTION
    WHEN OTHERS THEN
        -- Manejo de otras excepciones
...
BEGIN
    ...
    FROM PRODUCTS p
    where user_id is null;  -- Compara si el ID que llega es del dueï¿½o del pago
    RETURN v_cursor; -- Retornar el cursor con los registros
EXCEPTION
    WHEN OTHERS THEN
        -- Manejo de otras excepciones
...
BEGIN
    ...
    OPEN v_cursor FOR
    SELECT *
    FROM USERS;  -- Compara si el ID que llega es del dueï¿½o del pago
    RETURN v_cursor; -- Retornar el cursor con los registros
EXCEPTION
    WHEN OTHERS THEN
        -- Manejo de otras excepciones
          ...
          AND (
              ...
              (p_user_id IS NOT NULL AND (
                  ...
                  (P.USER_ID IS NOT NULL AND P.USER_ID IN (
                      ...
                      FROM USERS U
                      WHERE U.USER_ID = p_user_id -- Parï¿½metro del usuario actual
                  ))
              ))
          )
        ORDER BY P.CREATED_AT DESC
...
BEGIN
    ...
    AND cp.active = 1 AND c.active = 1;  -- Compara si el ID que llega es del dueï¿½o del pago
    RETURN v_cursor; -- Retornar el cursor con los registros
EXCEPTION
    WHEN OTHERS THEN
        -- Manejo de otras excepciones
...
BEGIN
    ...
    AND cp.active = 1 
    AND c.active = 1
    AND (p.user_id IS NULL OR u.user_id IS NOT NULL);  -- Condiciï¿½n: si el product no tiene user_id, no hacer JOIN
    RETURN v_cursor; -- Retornar el cursor con los registros
EXCEPTION
    WHEN OTHERS THEN
        -- Manejo de otras excepciones
...
BEGIN
    ...
    IF v_count > 0 THEN
        ...
        WHERE user_id = p_user_id AND name = p_name;
        COMMIT;
        RETURN 1; -- Retorna 1 si se actualizï¿½ exitosamente
    ELSE
        ...
        COMMIT;
        RETURN 2; -- Retorna 2 si se insertï¿½ exitosamente
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
...
BEGIN
    ...
    WHERE tp.seller_id = p_user_id;  -- Compara si el ID que llega es del dueï¿½o del pago
    RETURN v_cursor; -- Retornar el cursor con los registros
EXCEPTION
    WHEN OTHERS THEN
        -- Manejo de otras excepciones
...
BEGIN
    ...
    SELECT *
    FROM TRANSACTIONS_LIST
    WHERE buyer_id = p_user_id;  -- Compara si el ID que llega es del dueï¿½o del pago
    RETURN v_cursor; -- Retornar el cursor con los registros
EXCEPTION
    WHEN OTHERS THEN
        -- Manejo de otras excepciones
...
RETURN VARCHAR2 AS
BEGIN
    -- Construir la consulta dinï¿½mica para actualizar solo los campos no nulos
    UPDATE banking_entities
    SET 
